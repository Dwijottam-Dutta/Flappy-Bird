...
def welcomeScreen():
    ...
    gameIcon = pygame.image.load('./gallery/icon001.png')
    pygame.display.set_icon(gameIcon)
    messagey = int(SCREENHEIGHT * 0.1)
    GAME_SOUNDS['Yay'].play(-1)
    GAME_SOUNDS['Yaay'].stop()
    basex = 0
    while True:
...
def isCollide(playerx, playery, upperPipes, lowerPipes):
    ...
    for pipe in upperPipes:
        ...
        if playery < pipeHeight + pipe['y'] and abs(playerx - pipe['x']) < GAME_SPRITES['pipe'][-2].get_width():
            GAME_SOUNDS['hit'].play()
            return True

    for pipe in lowerPipes:
        if (playery + GAME_SPRITES['player'].get_height() > pipe['y']) and abs(playerx - pipe['x']) < \
                GAME_SPRITES['pipe'][-2].get_width():
            GAME_SOUNDS['hit'].play()
            return True
...
if __name__ == "__main__":
    ...
    pygame.display.set_caption('Dark Flappy Bird+')
    ...
    GAME_SPRITES['message'] = pygame.image.load(
        './gallery/virus.png').convert_alpha()
    ...
    GAME_SPRITES['pipe'] = (pygame.transform.rotate(pygame.image.load('./gallery/pipe.png').convert_alpha(), 180),
                            pygame.image.load(
    ...
    GAME_SOUNDS['Yay'] = pygame.mixer.Sound('./gallery/jazz.mp3')
    GAME_SOUNDS['Yaay'] = pygame.mixer.Sound('./gallery/jazzzz.mp3')
